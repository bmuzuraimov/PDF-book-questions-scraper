...
class App(customtkinter.CTk):
    ...
    def __init__(self, gui_vars):
        ...
        for row, topic in enumerate(self.topics, 2):
            topic_name = self.text2label(topic[2], 4)
            self.button_1 = customtkinter.CTkButton(master=self.frame_left,
                                                    text=topic_name,
                                                    text_font=("Arial", 8),
                                                    command=self.button_event)
            self.button_1.grid(row=row, column=0, pady=10, padx=20)
        ...
        my_question = self.questions[5]
        my_options = []
        if(my_question[3] == 1):
            for option in self.options:
                if(option[1] == my_question[0]):
                    my_options.append(option)
        question = self.text2label(my_question[2], 6)
        ...
        for index, option in enumerate(my_options):
            option_name = option[2] + ' ' + option[3]
            self.radio_button_1 = customtkinter.CTkRadioButton(master=self.frame_right,
                                                               variable=self.radio_var,
                                                               text=option_name,
                                                               value=0)
            self.radio_button_1.grid(row=index+1, column=2, pady=10, padx=20, sticky="n")
    ...
    def text2label(self, text, nl_pos):
        new_text = ''
